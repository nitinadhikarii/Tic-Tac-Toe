...
def make_move(board,symbol,move):
    ...
    if 1 <= move <= 9:
        ...
        if board[row][column] in list_of_symbols:
                ...
                if symbol=="o":
                    make_move(board,symbol,random.randint(1,9))
                else:
                    try:
...
#validation for symbol "o"

p1.symbol = a.split(",")[1].strip()
while p1.symbol == "o":      
    print("symbol 'o' is reserved for the computer!")  
    p1.symbol = input("enter a new symbol: ")
print(p1.statement()) 
...
while True:
    ...
    move2= random.randint(1,9)#int(input("player 2 move (enter 0 to quit): "))
